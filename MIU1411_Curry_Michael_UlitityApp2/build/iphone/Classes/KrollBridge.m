...
NSString * MIU1411_Curry_Michael_UlitityApp2$ModuleRequireFormat = @"(function(exports){"
...
@implementation MIU1411_Curry_Michael_UlitityApp2Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_miu1411_curry_michael_ulitityapp2);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_miu1411_curry_michael_ulitityapp2 gc];
}
...
{
	// create MIU1411_Curry_Michael_UlitityApp2 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "MIU1411_Curry_Michael_UlitityApp2" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_miu1411_curry_michael_ulitityapp2 = [[MIU1411_Curry_Michael_UlitityApp2Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_miu1411_curry_michael_ulitityapp2];
	
	NSString *_miu1411_curry_michael_ulitityapp2NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _miu1411_curry_michael_ulitityapp2NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_miu1411_curry_michael_ulitityapp2 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:MIU1411_Curry_Michael_UlitityApp2$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.miu1411_curry_michael_ulitityapp2.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.miu1411_curry_michael_ulitityapp2.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
