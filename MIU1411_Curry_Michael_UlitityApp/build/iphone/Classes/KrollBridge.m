...
NSString * MIU1411_Curry_Michael_UlitityApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation MIU1411_Curry_Michael_UlitityAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_miu1411_curry_michael_ulitityapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_miu1411_curry_michael_ulitityapp gc];
}
...
{
	// create MIU1411_Curry_Michael_UlitityApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "MIU1411_Curry_Michael_UlitityApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_miu1411_curry_michael_ulitityapp = [[MIU1411_Curry_Michael_UlitityAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_miu1411_curry_michael_ulitityapp];
	
	NSString *_miu1411_curry_michael_ulitityappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _miu1411_curry_michael_ulitityappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_miu1411_curry_michael_ulitityapp valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:MIU1411_Curry_Michael_UlitityApp$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.miu1411_curry_michael_ulitityapp.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.miu1411_curry_michael_ulitityapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
