...
//Rare exceptions to indicate a bug in the _miu1411_curry_michael_ulitityapp code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_miu1411_curry_michael_ulitityapp = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_miu1411_curry_michael_ulitityapp]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.miu1411_curry_michael_ulitityapp.module"
